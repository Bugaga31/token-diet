...
_NEGATIVE_REACTION = re.compile(
    r"\b(упал|упала|упали|снизил|снизила|снизились|падение|просела|просел|"
    r"разочар|разочарование|продаж|распродаж|коррекци|откат|откатил|"
    r"инсайдер|продавал|продавали|не оправд|ниже прогноз|слаб|слабо|"
    r"убыточ|долг растёт|свободный денежный поток страдает)\b",
    re.IGNORECASE,
)
_POSITIVE_REACTION = re.compile(
    r"\b(вырос|выросла|выросли|рост|рекорд|рекордн|максимум|позитивн|превзо|"
    r"лучше прогноз|дивид|выкупает|buyback|байбэк|целевая цена|повыш|апгрейд|"
    r"buy|обнов.*макс|превыс)\b",
    re.IGNORECASE,
)
