...
def check_entry(inv: TinkoffInvest) -> str:
    ...
    # ── АРМИЯ: второе мнение перед покупкой (СОВЕТНИК, не владелец) ──
    # Урок генерала 14.08: «У них нет такой же инфы как у тебя —
    # большую часть решения принимаешь ты». Армия не имеет стаканов,
    # Пульса, ТГ-новостей, геополитики в реальном времени.
    # Если мои данные СИЛЬНЫЕ (скор >= 75) — решаю сам, даже если
    # армия против. Если данные СРЕДНИЕ (60-75) — армия может
    # переубедить, жду подтверждения.
    cand_score = float(decision.checks.get("cand_score", 0.0) or 0.0)
    army_ok = True
    try:
    ...
    if not army_ok and cand_score < 75:
        # Армия против И мои данные средние → ждём.
        log(f"армия ПРОТИВ {ticker} и скор {cand_score:.0f} < 75 — ждём "
            f"подтверждения (данные не дотягивают)")
        return "wait"
    if not army_ok:
        # Армия против, но мои данные СИЛЬНЫЕ → решаю сам (урок 3).
        log(f"армия против {ticker}, но скор {cand_score:.0f} >= 75 — "
            f"решаю сам (урок генерала: у меня больше инфы)")
